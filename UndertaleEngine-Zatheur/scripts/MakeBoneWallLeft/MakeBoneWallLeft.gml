...
function MakeBoneWallLeft() {
	...
	for(i=0;i<battle_board.up+battle_board.down+11;i+=11){
		ii += 1;
		b[ii] = MakeBoneH11(battle_board.x-battle_board.left-LENGTH/30,battle_board.y-battle_board.up+i,LENGTH,0,0,BONE.WHITE,0,0,0,1,DURATION+PAUSE*2);
		with(b[ii]){
			Anim_Create(id,"x",0,0,x,+LENGTH,8,PAUSE);
			Anim_Create(id,"x",0,0,x+LENGTH,-LENGTH*5,16,16+PAUSE+DURATION);
