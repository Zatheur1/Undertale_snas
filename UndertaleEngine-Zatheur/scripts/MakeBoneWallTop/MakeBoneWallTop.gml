...
function MakeBoneWallTop() {
	...
	for(i=-3;i<battle_board.left+battle_board.right+11;i+=11){
		ii += 1;
		b[ii] = MakeBoneV(battle_board.x-battle_board.left+i,battle_board.y-battle_board.up-LENGTH/30,LENGTH,0,0,BONE.WHITE,0,0,0,1,DURATION+PAUSE*2);
		with(b[ii]){
			Anim_Create(id,"y",0,0,y,LENGTH,8,PAUSE);
			Anim_Create(id,"y",0,0,y+LENGTH,-LENGTH*5,16,16+PAUSE+DURATION);
